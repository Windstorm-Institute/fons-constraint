...
\begin{equation}
    m \geq \frac{\log_2(C)}{\log_2(A) \cdot C_{\mathrm{channel}}}
\end{equation}
...
Derived from & Shannon + Eigen & Eigen~\cite{eigen1971} \\
...
AI (frontier) & Token & $\sim$100K & $\sim$4.6 bits \\
\bottomrule
\end{tabular}
\end{table}
...
The broader constraint appears to operate at the effective throughput level ($\sim$4--5 bits/step), with vocabulary size as a dependent variable. This is consistent with Miller's~\cite{miller1956} finding on working memory and Cowan's~\cite{cowan2001} revised estimate. The throughput convergence is examined formally in companion papers~\cite{whitmer2026rlf,whitmer2026tb,whitmer2026sdb,whitmer2026dd,whitmer2026ic}.
